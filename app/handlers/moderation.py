...
async def process_approve(callback: CallbackQuery, session: AsyncSession, bot: Bot):
    ...
    event = await update_event_status(session, event_id, EventStatus.APPROVED, moderator)
    if not event:
    ...
    await callback.message.answer(f"✅ Event #{event_id} ('{event.title}') has been approved.")
...
async def process_reject(callback: CallbackQuery, session: AsyncSession):
    ...
    event = await update_event_status(session, event_id, EventStatus.REJECTED, moderator)
    if not event:
        await callback.answer("Event not found.", show_alert=True)
        return
...
async def process_changes(callback: CallbackQuery, session: AsyncSession):
    ...
    event = await update_event_status(session, event_id, EventStatus.NEEDS_CHANGES, moderator)
    if not event:
        await callback.answer("Event not found.", show_alert=True)
        return
