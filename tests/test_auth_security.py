...
async def test_telegram_edit_event_ownership(mock_settings):
    ...
    with patch("app.handlers.event_edit.get_settings", return_value=mock_settings), \
...
async def test_telegram_delete_event_ownership(mock_settings):
    ...
    with patch("app.handlers.user_events.get_settings", return_value=mock_settings), \
...
async def test_telegram_moderation_requires_role(mock_settings):
    ...
    with patch("app.handlers.moderation.get_settings", return_value=mock_settings):
        cb.answer = AsyncMock()
